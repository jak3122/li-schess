<template>
  <div id="app">
    <div class="header">
      ...
      <div class="stats">
        <div><router-link to="/players">{{ numConnectionsText }}</router-link></div>
...
export default {
	...
	computed: {
		numConnectionsText: function() {
			return `${this.numConnections} user${this.numConnections === 1
				? ""
				: "s"} connected`;
		},
		numGamesText: function() {
			return `${this.numGames} game${this.numGames === 1
				? ""
				: "s"} in play`;
		}
	},
...
<style>
body {
	font: 12px "Noto Sans", "Lucida Grande", "Lucida Sans Unicode", Geneva,
		Verdana, Sans-Serif;
	margin: 0px;
}
